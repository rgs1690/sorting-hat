...
// create render to Dom function, 
const renderToDom =(divId, textToRender) => {
    const selectedDiv= document.querySelector(divId);   // selects the div id that is on the dom where you want to print
    selectedDiv.innerHTML = textToRender; // uses .innerHtml to place whatever it is you want to render to the dom
};
...
const createStudentCard = (array) =>{
    let domString=""
    array.forEach((student, i) => {       //loops through student array to build the studets, i is used for the expel button
    domString += `
    <div class=${student.house} style="width: 18rem;">
...
const expelledStudents = (array) => {
   ...
   // handles button clicks for each house and prints the student cards only if the are in said house
    ...
    const sortAlpha = (array) => {
        const alphaCards = array.sort(function(name1, name2){
            const firstName = 
            name1.toString().toLowerCase();
            const secondName = 
            name2.toString().toLowerCase(); 
